...
| 1.1     | 2026-07-23 | Agent                   | Native service artifact realized: standalone `native-service` Rust binary (links `rustpush`) packaged as `librustpush_service.so`, built via `mise run build:rust`. Implements the abstract-socket length-prefixed JSON transport and `PING`/`PONG` heartbeat; `ACTIVATE`/`SEND_MESSAGE`/`GET_MESSAGES` handlers stubbed (return structured `ERROR`) pending Apple-facing wiring. Replaces the earlier `rust-android-gradle` cdylib approach (rustpush is an rlib, not a cdylib). |
