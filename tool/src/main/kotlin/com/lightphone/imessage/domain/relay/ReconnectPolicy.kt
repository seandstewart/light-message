 ...
 * baseDelayMs, capped at maxDelayMs, plus a random jitter in [0, baseDelayMs) to reduce thundering
 * herd on shared outages. Spec: milestone-2.md § 6.4 (Relay Reconnect with Backoff).
 *
 * Example with defaults (before jitter):
...
data class ReconnectPolicy(
     ...
     * attempt), capped at maxDelayMs, plus random jitter in [0, baseDelayMs).
     *
    ...
    fun getDelayMs(attempt: Int): Long {
        ...
        val capped = delay.coerceAtMost(maxDelayMs)
        val jitter = if (baseDelayMs > 0) Random.nextLong(0L, baseDelayMs) else 0L
